...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      const redirectPath = user.role === 'admin' ? '/admin/companies' : '/dashboard';
    ...
    onSubmit: async (values) => {
      try {
        ...
        setTimeout(() => {
          ...
          const redirectPath = userRole === 'admin' ? '/admin/companies' : '/dashboard';
        ...
        const errorMessage = err?.data?.message || err?.error || err?.message || 'Failed to login. Please check your credentials.';
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="backdrop-blur-xl bg-white/90 border border-blue-200 p-8 sm:p-10 rounded-3xl shadow-2xl transition-all duration-300">
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-6">
              {/* Email Field */}
              <div>
                <label className="block text-xs font-semibold text-gray-700 uppercase tracking-widest mb-2" htmlFor="email">
                  Email Address
                </label>
              ...
              {/* Password Field */}
              <div>
                <label className="flex justify-between text-xs font-semibold text-gray-700 uppercase tracking-widest mb-2" htmlFor="password">
                  <span>Password</span>
                  <Link href="/forgot-password" className="text-blue-600 hover:text-blue-800 capitalize tracking-normal underline decoration-blue-600/30 underline-offset-2 transition-colors">
                    Forgot?
                  </Link>
                </label>
                <div className="relative">
            ...
            <div className="mt-8 text-center text-xs text-gray-600">
              ...
              <Link href="/register" className="text-blue-600 font-medium hover:text-blue-800 transition-colors underline decoration-blue-600/30 underline-offset-4">
                Create one now
              </Link>
            </div>
          </div>
